...
def compute_psd(segmentation):
    ...
    def manual_feret_diameter_and_surface_area(prop):
        """
        Compute the Feret diameter (maximum caliper distance) and surface area in voxels using the convex hull
        and marching cubes from an existing regionprops object.

        Args:
            prop (skimage.measure._regionprops.RegionProperties): Region property from skimage.measure.regionprops.

        Returns:
            tuple: Maximum Feret diameter (in voxels), surface area (in voxel units).
        """
        # Ensure the convex hull image is available
        if not hasattr(prop, 'image_convex'):
            raise ValueError("Region property does not have a convex hull image.")

        # Step 1: Pad the convex hull to avoid boundary issues
        identity_convex_hull = np.pad(prop.image_convex, 2, mode='constant', constant_values=0)

        # Step 2: Perform marching cubes to extract surface points and faces
        verts, faces, _, _ = measure.marching_cubes(identity_convex_hull, level=0.5)

        # Step 3: Compute pairwise distances to calculate the Feret diameter
        distances = pdist(verts, 'sqeuclidean')
        feret_diameter_voxels = sqrt(np.max(distances))

        # Step 4: Compute the surface area of the mesh using skimage's mesh_surface_area
        surface_area_voxels = measure.mesh_surface_area(verts, faces)

        return feret_diameter_voxels, surface_area_voxels
    ...
    def compute_metrics(prop):
        ...
        feret, surface_area = manual_feret_diameter_and_surface_area(prop)
